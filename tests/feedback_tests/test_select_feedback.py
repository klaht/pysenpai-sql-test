...
def test_incorrect_column():
    ans_query = """
        SELECT artist.name, location.city, exhibition.title FROM artist 
        JOIN artwork ON artist.artistID = artwork.artistID 
        JOIN on_exhibition ON artwork.artworkID = on_exhibition.artworkID 
        JOIN exhibition ON on_exhibition.exhibitionID = exhibition.exhibitionID 
        JOIN location ON exhibition.locationID = location.locationID 
    ...
    """
    ref_query = """
        SELECT artist.name, exhibition.title, location.city FROM artist 
        JOIN artwork ON artist.artistID = artwork.artistID 
        JOIN on_exhibition ON artwork.artworkID = on_exhibition.artworkID 
        JOIN exhibition ON on_exhibition.exhibitionID = exhibition.exhibitionID 
        JOIN location ON exhibition.locationID = location.locationID 
        ...
        ORDER BY artist.name ASC, exhibition.title ASC; 
    """
    args = ["exNumber = 0", "show_answer_difference", "feedback=selected_columns"]
    ...
    returned_msgs = parse_flag_msg(output, 2)
    error_keys = ["unmatchedColumn", "AdditionalTests"]
    correct_msgs = get_msg("en", error_keys)
    assert compare_messages(returned_msgs, correct_msgs)

    assert not answer

def test_incorrect_column_amount():
    ans_query = """
        SELECT artist.name, location.city FROM artist 
        JOIN artwork ON artist.artistID = artwork.artistID 
        JOIN on_exhibition ON artwork.artworkID = on_exhibition.artworkID 
        JOIN exhibition ON on_exhibition.exhibitionID = exhibition.exhibitionID 
        JOIN location ON exhibition.locationID = location.locationID 
    ...
    returned_msgs = parse_flag_msg(output, 2)
    error_keys = ["incorrectSelectedColumnAmount", "AdditionalTests"]
    correct_msgs = get_msg("en", error_keys)
    assert compare_messages(returned_msgs, correct_msgs)
